...
class Platform(object):
    ...
    @staticmethod
    def get_gpu_mem_size(self, id):
        return singa.Platform.GetGPUMemSize(id)

    @staticmethod
    def device_query(self, id, verbose=False):
        return singa.Platform.DeviceQuery(id, verbose)

    @staticmethod
    def create_raw_cuda_gpus(self, num):
        return singa.Platform.CreateCudaGPUs(num)


class CudaGPU(Device):
    def __init__(self, id=0):
        super(CudaGPU, self).__init__(id, Platform.create_raw_cuda_gpus(1)[0])
